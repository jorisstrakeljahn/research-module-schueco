...
app/
  ...
  main.py              # FastAPI app
  cli.py               # `trendscout` CLI (init-db | run | serve)
  api/routes.py        # /health /runs /trends /trends/{id} /trends/{id}/feedback
  ingestion/           # source connectors (OpenAlex; Connector protocol)
  pipeline/            # embeddings, topics, timeseries, describe, run (orchestration)
